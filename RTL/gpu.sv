...
module gpu #(
    ...
    controller #(
        ...
        .mem_read_data(program_mem_read_data) // Fixed trailing comma
    );
    ...
    // Compute Cores Generation
    genvar i, j;
    generate
        for (i = 0; i < NUM_CORES; i = i + 1) begin : cores
            
            // Internal signals for this specific core
            ...
            // Combinatorial mapping between Core LSUs and Global Memory Controller
            for (j = 0; j < THREADS_PER_BLOCK; j = j + 1) begin : lsu_mapping
                localparam lsu_idx = i * THREADS_PER_BLOCK + j;
                
                assign lsu_read_valid[lsu_idx]    = core_lsu_read_valid[j];
                assign lsu_read_address[lsu_idx]  = core_lsu_read_address[j];
                assign lsu_write_valid[lsu_idx]   = core_lsu_write_valid[j];
                assign lsu_write_address[lsu_idx] = core_lsu_write_address[j];
                assign lsu_write_data[lsu_idx]    = core_lsu_write_data[j];

                assign core_lsu_read_ready[j]     = lsu_read_ready[lsu_idx];
                assign core_lsu_read_data[j]      = lsu_read_data[lsu_idx];
                assign core_lsu_write_ready[j]    = lsu_write_ready[lsu_idx];
            end
            ...
            core #(
                ...
                // Program Memory connection for this core
                .program_mem_read_valid(fetcher_read_valid[i]),
                .program_mem_read_address(fetcher_read_address[i]),
                .program_mem_read_ready(fetcher_read_ready[i]),
                .program_mem_read_data(fetcher_read_data[i]),

                // Data Memory connection for this core (multiple LSUs)
